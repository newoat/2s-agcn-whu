...
class unit_gcn(nn.Module): #spatial GCN
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, num_subset=3): #3 subsets: self, centripetal, centrifugal
        ...
        #####################################################
        #self.Ck = nn.Parameter(torch.from_numpy(A.astype(np.float32)))
        #self.ABC = nn.Parameter(torch.from_numpy(A.astype(np.float32)))
        #nn.init.constant_(self.Ck, 1e-6)
        #nn.init.constant_(self.ABC, 1e-6)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_subset):#kv=3
            ...
            ################
            #print(A1.size()) #(128,25,25)
            #print(A[i].size()) #(25,25)
            #self.Ck[i] = A1
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.data_bn(x)
        x = x.view(N, M, V, C, T).permute(0, 3, 4, 1, 2).contiguous().view(N, C, T, M*V) #permute(0,3,4,1,2): N,C,T,M,V

        x = self.l1(x) #outsize = N,64,T,MV
        x = self.l2(x) #outsize = N,64,T,MV
        x = self.l3(x) #outsize = N,64,T,MV
        x = self.l4(x) #outsize = N,64,T,MV
        x = self.l5(x) #outsize = N,128,T/2,MV
        x = self.l6(x) #outsize = N,128,T,MV
        x = self.l7(x) #outsize = N,128,T,MV
        x = self.l8(x) #outsize = N,256,T/4,MV
        x = self.l9(x) #outsize = N,256,T/4,MV
        x = self.l10(x) #outsize = N,256,T/4,MV

        # N,C,T,MV
        c_new = x.size(1) #new channel, 256
        x = x.view(N, c_new, -1) #-1:actual value for this dimension will be inferred. #N,256,TMV/4
        x = x.mean(2) #mean(3)??? ave of TV? #mean(1): average of 2 person. #N,256
